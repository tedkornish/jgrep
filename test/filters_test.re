...
let module Parse = {
  let test1 ctx =>
    assert_equal (parseFilter "age is 9") (Some (Pred (Field "age") (Equal (Num 9.0))));
  let test2 ctx =>
    assert_equal (parseFilter "age = 9") (Some (Pred (Field "age") (Equal (Num 9.0))));
  let test3 ctx => assert_equal (parseFilter "age > 9") (Some (Pred (Field "age") (GT 9.0)));
  let test4 ctx =>
    assert_equal (parseFilter "age is greater than 9") (Some (Pred (Field "age") (GT 9.0)));
  let test5 ctx =>
    assert_equal (parseFilter "age greater than 9") (Some (Pred (Field "age") (GT 9.0)));
  let test6 ctx =>
    assert_equal
      (parseFilter "age matches /^bob(by)?/")
      (Some (Pred (Field "age") (Matches (Regex "^bob(by)?"))));
  let test7 ctx => assert_equal (parseFilter "age < -10") (Some (Pred (Field "age") (LT (-10.0))));
  let test8 ctx =>
    assert_equal (parseFilter "age is less than .09") (Some (Pred (Field "age") (LT 0.09)));
  let test9 ctx =>
    assert_equal (parseFilter "age less than 9.56") (Some (Pred (Field "age") (LT 9.56)));
  let test10 ctx =>
    assert_equal
  ...
  let test12 ctx =>
    assert_equal
      (parseFilter "state is colorado") (Some (Pred (Field "state") (Equal (String "colorado"))));
  let test13 ctx => assert_equal (parseFilter "age greater than hello") None;
  let test14 ctx =>
    assert_equal (parseFilter "msg contains Hello") (Some (Pred (Field "msg") (Contains "Hello")));
  let test15 ctx =>
    assert_equal
  ...
  let test17 ctx =>
    assert_equal
      printer::[%show : option predicate]
      (parseFilter "(msg contains hello)")
