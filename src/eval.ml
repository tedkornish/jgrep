...
let rec to_jq_filter filter = match filter with
  ...
  | Contains s -> sprintf "|ascii_downcase|contains(\"%s\"|ascii_downcase)" s
  | Matches (Regex r) -> sprintf "|test(\"%s\";i)" r
  | BeginsWith s -> sprintf "|ascii_downcase|startswith(\"%s\"|ascii_downcase)" s
  | EndsWith s -> sprintf "|ascii_downcase|endswith(\"%s\"|ascii_downcase)" s
...
let to_jq (Exp (pred, selectors)) =
  ...
  sprintf "jq --unbuffered -r -c '%s'" if_then_stmt
